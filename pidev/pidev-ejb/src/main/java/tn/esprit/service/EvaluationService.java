...
public List<Evaluation> getAllEvaluationLaunche() {
	TypedQuery<Evaluation>query=em.createQuery("select e from Evaluation e where e.Launch=1 and e.Traiter=0",Evaluation.class
		);
...
public List<Evaluation> getAllEvaluationTraiter() {
	TypedQuery<Evaluation>query=em.createQuery("select e from Evaluation e where e.Traiter=1 ",Evaluation.class
		);
List<Evaluation> result=query.getResultList();
	return result;
}
...
public void updateEvaluationCritere(int id, String i) {
	em.createQuery("update Critere  set description = i where evaluation.id = :id ").setParameter("id",id).executeUpdate();
	
}

public void updatecritereniveau(String c, String niveau,int id) {
	System.out.println("je suis la");
	em.createQuery("update Critere  set niveau = ?   where id = :id ").setParameter(1, niveau).setParameter("id",id).executeUpdate();
}
